...
*/
  std::string filename;
  while (std::getline(infile, line)) {
    int pos = line.find_last_of(".jpg'");
    if (pos < 0 )
      pos = line.find_last_of(".png'");
      if (pos < 0)
        pos = line.find_last_of(".gif'");
    if (pos < 0) {
       std::cout<<"Not found image format, ignore: "<<line<<std::endl;
       continue;
    }
    filename = line.substr(0, pos+1);
    std::cout<<filename<<" ";
