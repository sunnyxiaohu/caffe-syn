...
using std::min;
using std::pair;
bool compair(const pair<int,float>& lp, const pair<int,float>& rp) {
  return lp.second > rp.second; // > ,　sort desc
}
...
namespace caffe {  
  ...
  void BoxAnnotatorOHEMLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    // use map and vector instead of C++11 feature
    vector<pair<int,float> > sorted_idx(num_rois_);
    for (int i = 0; i < num_rois_; i++){
      sorted_idx[i] = make_pair(i, float(bottom_loss[i]));
    }
//TODO: 
    std::sort(sorted_idx.begin(), sorted_idx.end(), compair);
//      [bottom_loss](int i1, int i2){return bottom_loss[i1] > bottom_loss[i2]; });

// use nms for rois via loss

    // Generate output labels for scoring and loss_weights for bbox regression
    vector<int> number_left(num_imgs, roi_per_img_);
    for (int i = 0; i < num_rois_; i++){
      int index = sorted_idx[i].second;
